...
{
    s16 *data = gTasks[taskId].data;
    ConvertIntToDecimalStringN(gStringVar3, ItemId_GetPrice(BagGetItemIdByPocketPosition(gBagMenuState.pocket + 1, data[1])) / 4 * data[8], STR_CONV_MODE_LEFT_ALIGN, 6);
...
static void Task_InitSaleQuantitySelectInterface(u8 taskId)
{
    ...
    UpdateSalePriceDisplay(ItemId_GetPrice(BagGetItemIdByPocketPosition(gBagMenuState.pocket + 1, data[1])) / 4 * data[8]);
    BagPrintMoneyAmount();
...
static void Task_FinalizeSaleToShop(u8 taskId)
{
    ...
    PlaySE(SE_SHOP);
    RemoveBagItem(gSpecialVar_ItemId, data[8]);
    AddMoney(&gSaveBlock1Ptr->money, ItemId_GetPrice(gSpecialVar_ItemId) / 4 * data[8]);
