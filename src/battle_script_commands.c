...
static void Cmd_jumpifstatus4condition(void);
...
void (* const gBattleScriptingCommandsTable[])(void) =
{
    ...
    Cmd_jumpifstat,                              //0x20
    Cmd_jumpifstatus4condition,                  //0x21
    Cmd_jumpiftype,                              //0x22
    Cmd_getexp,                                  //0x23
...
{
    if (gstatuses4[gBattlerTarget] & STATUS4_ALWAYS_HITS && gDisableStructs[gBattlerTarget].battlerWithSureHit == gBattlerAttacker)
    {
        JumpIfMoveFailed(7, move);
        return TRUE;
    }

    if (!(gHitMarker & HITMARKER_IGNORE_ON_AIR) && gstatuses4[gBattlerTarget] & STATUS4_ON_AIR)
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        JumpIfMoveFailed(7, move);
        return TRUE;
    ...
    if (!(gHitMarker & HITMARKER_IGNORE_UNDERGROUND) && gstatuses4[gBattlerTarget] & STATUS4_UNDERGROUND)
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        JumpIfMoveFailed(7, move);
        return TRUE;
    ...
    if (!(gHitMarker & HITMARKER_IGNORE_UNDERWATER) && gstatuses4[gBattlerTarget] & STATUS4_UNDERWATER)
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
        JumpIfMoveFailed(7, move);
        return TRUE;
...
static void Cmd_accuracycheck(void)
{
    ...
    {
        if (gstatuses4[gBattlerTarget] & STATUS4_ALWAYS_HITS && move == NO_ACC_CALC_CHECK_LOCK_ON && gDisableStructs[gBattlerTarget].battlerWithSureHit == gBattlerAttacker)
            gBattlescriptCurrInstr += 7;
        else if (gstatuses4[gBattlerTarget] & (STATUS4_ON_AIR | STATUS4_UNDERGROUND | STATUS4_UNDERWATER))
    ...
    else
    {
        ...
        if (hasActiveAbility(gBattlerAttacker, ABILITY_COMPOUND_EYES))
...
static void Cmd_critcalc(void)
{
    ...
    critChance  = 3 * ((gBattleMons[gBattlerAttacker].status2 & STATUS2_FOCUS_ENERGY) != 0)
                + (gBattleMoves[gCurrentMove].effect == EFFECT_HIGH_CRITICAL)
                + (gBattleMoves[gCurrentMove].effect == EFFECT_BLAZE_KICK)
                + (gBattleMoves[gCurrentMove].effect == EFFECT_POISON_TAIL)
                + (holdEffect == HOLD_EFFECT_SCOPE_LENS)
                + hasActiveAbility(gBattlerAttacker, ABILITY_SUPER_LUCK)
...
static void Cmd_damagecalc(void)
{
    ...
    if (gstatuses4[gBattlerAttacker] & STATUS4_CHARGED_UP && gBattleMoves[gCurrentMove].type == TYPE_ELECTRIC)
        gBattleMoveDamage *= 2;
...
void AI_CalcDmg(u8 attacker, u8 defender)
{
    ...
    if (gstatuses4[attacker] & STATUS4_CHARGED_UP && gBattleMoves[gCurrentMove].type == TYPE_ELECTRIC)
        gBattleMoveDamage *= 2;
...
u8 TypeCalc(u16 move, u8 attacker, u8 defender)
{
    ...
    else if (gBattleMoves[move].effect != EFFECT_RAZOR_WIND)
    {
        while (TYPE_EFFECT_ATK_TYPE(i) != TYPE_ENDTABLE)
        {
            if (TYPE_EFFECT_ATK_TYPE(i) == moveType)
    ...
    if (hasActiveAbility(defender, ABILITY_WONDER_GUARD) && !(flags & MOVE_RESULT_MISSED)
...
static void Cmd_resultmessage(void)
{
    ...
    else
    {
        ...
        default:
            ...
            {
                gMoveResultFlags &= ~(MOVE_RESULT_FOE_ENDURED | MOVE_RESULT_FOE_HUNG_ON | MOVE_RESULT_FOE_STURDIED);
                BattleScriptPushCursor();
            ...
            else if (gMoveResultFlags & MOVE_RESULT_FOE_HUNG_ON)
            {
                gLastUsedItem = gBattleMons[gBattlerTarget].item;
                gPotentialItemEffectBattler = gBattlerTarget;
                gMoveResultFlags &= ~(MOVE_RESULT_FOE_HUNG_ON | MOVE_RESULT_FOE_STURDIED);
                BattleScriptPushCursor();
                gBattlescriptCurrInstr = BattleScript_FocusBandActivates;
                return;
            }
...
void SetMoveEffect(bool8 primary, u8 certain)
{
    ...
    if (gBattleCommunication[MOVE_EFFECT_BYTE] <= PRIMARY_STATUS_MOVE_EFFECT)
    {
        ...
        case STATUS1_SLEEP:
            // check active uproar
            if (!hasActiveAbility(gEffectBattler, ABILITY_SOUNDPROOF))
            {
            ...
            if (gActiveBattler != gBattlersCount)
                break;
            if (hasActiveAbility(gEffectBattler, ABILITY_VITAL_SPIRIT))
                break;
            if (hasActiveAbility(gEffectBattler, ABILITY_INSOMNIA))
                break;
            if (hasActiveAbility(gEffectBattler, ABILITY_LEAF_GUARD)
            ...
            break;
        case STATUS1_POISON:
            if ((hasActiveAbility(gEffectBattler, ABILITY_IMMUNITY)
            ...
            break;
        case STATUS1_BURN:
            if ((hasActiveAbility(gEffectBattler, ABILITY_WATER_VEIL)
                    || (hasActiveAbility(gEffectBattler, ABILITY_LEAF_GUARD)
                               && WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_SUN)))
                && (primary == TRUE || certain == MOVE_EFFECT_CERTAIN))
            {
        ...
        case STATUS1_FREEZE:
            ...
            if (noSunCanFreeze == FALSE)
                break;
            if (hasActiveAbility(gEffectBattler, ABILITY_MAGMA_ARMOR))
                break;
            if (hasActiveAbility(gEffectBattler, ABILITY_LEAF_GUARD)
                    && WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_SUN))
                break;
        ...
        case STATUS1_PARALYSIS:
            if (hasActiveAbility(gEffectBattler, ABILITY_LIMBER)
                    || (hasActiveAbility(gEffectBattler, ABILITY_LEAF_GUARD)
                               && WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_SUN)))
            {
    ...
    else
    {
        ...
        else
        {
            ...
            case MOVE_EFFECT_STEAL_ITEM:
                {
                        ...
                        && (gWishFutureKnock.knockedOffMons[side] & gBitTable[gBattlerPartyIndexes[gBattlerAttacker]]))
                    {
                        ...
                        && hasActiveAbility(gBattlerTarget, ABILITY_STICKY_HOLD))
                    {
            ...
            case MOVE_EFFECT_KNOCK_OFF:
                if (hasActiveAbility(gEffectBattler, ABILITY_STICKY_HOLD))
                {
...
static void Cmd_seteffectwithchance(void)
{
    ...
    if (hasActiveAbility(gBattlerAttacker, ABILITY_SERENE_GRACE))
...
static void Cmd_tryfaintmon(void)
{
    ...
    else
    {
         ...
         && gBattleMons[gActiveBattler].hp == 0)
        {
            ...
            if ((gstatuses4[gBattlerTarget] & STATUS4_GRUDGE)
...
static void Cmd_jumpifability(void)
{
    ...
    else
    {
        ...
        if (hasActiveAbility(battlerId, ability))
        {
            gLastUsedAbility = ability;
            gBattlescriptCurrInstr = jumpPtr;
...
static void Cmd_jumpifstatus4condition(void)
{
    ...
    {
        if ((gstatuses4[gActiveBattler] & status) != 0)
...
static void Cmd_playanimation(void)
{
    ...
    else if (gHitMarker & HITMARKER_NO_ANIMATIONS)
    {
        ...
        gBattlescriptCurrInstr = BattleScript_Pausex20;
          ...
          || gBattlescriptCurrInstr[2] == B_ANIM_ASH_CONTINUES)
    {
        BtlController_EmitBattleAnimation(BUFFER_A, gBattlescriptCurrInstr[2], *argumentPtr);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr += 7;
    }
    else if (gstatuses4[gActiveBattler] & STATUS4_SEMI_INVULNERABLE)
    {
        gBattlescriptCurrInstr += 7;
    }
    else
...
static void Cmd_playanimation_var(void)
{
    ...
    else if (gHitMarker & HITMARKER_NO_ANIMATIONS)
    {
        gBattlescriptCurrInstr += 10;
          ...
          || *animationIdPtr == B_ANIM_ASH_CONTINUES)
    {
        BtlController_EmitBattleAnimation(BUFFER_A, *animationIdPtr, *argumentPtr);
        MarkBattlerForControllerExec(gActiveBattler);
        gBattlescriptCurrInstr += 10;
    ...
    do
    {
        ...
        case MOVEEND_ITEM_EFFECTS_ALL: // item effects for all battlers
            if (ItemBattleEffects(ITEMEFFECT_MOVE_END, 0, FALSE))
                ...
                gBattleScripting.moveendState++;
            break;
        case MOVEEND_ON_DAMAGE_ITEMS:
            if (ItemBattleEffects(ITEMEFFECT_ON_DAMAGE, 0, FALSE))
                effect = TRUE;
            gBattleScripting.moveendState++;
            break;
        ...
        case MOVEEND_ATTACKER_INVISIBLE: // make attacker sprite invisible
            if (gstatuses4[gBattlerAttacker] & (STATUS4_SEMI_INVULNERABLE)
        ...
        case MOVEEND_ATTACKER_VISIBLE: // make attacker sprite visible
            if (gMoveResultFlags & MOVE_RESULT_NO_EFFECT
                || !(gstatuses4[gBattlerAttacker] & (STATUS4_SEMI_INVULNERABLE))
        ...
        case MOVEEND_TARGET_VISIBLE: // make target sprite visible
            if (!gSpecialStatuses[gBattlerTarget].restoredBattlerSprite && gBattlerTarget < gBattlersCount
                && !(gstatuses4[gBattlerTarget] & STATUS4_SEMI_INVULNERABLE))
            {
                gActiveBattler = gBattlerTarget;
                BtlController_EmitSpriteInvisibility(BUFFER_A, FALSE);
                MarkBattlerForControllerExec(gActiveBattler);
...
static void Cmd_typecalc2(void)
{
    ...
    if (hasActiveAbility(gBattlerTarget, ABILITY_WONDER_GUARD)
...
static void Cmd_switchineffects(void)
{
        ...
        && !isAirborne(gActiveBattler))
    {
                ...
                || gBattleMons[gActiveBattler].type2 == TYPE_POISON)
        {
            ...
            gBattlescriptCurrInstr = BattleScript_RemoveToxicSpikes;
        }
        else
        {
            gEffectBattler = gActiveBattler;
            if (gBattlescriptCurrInstr[1] == BS_TARGET)
                gBattlescriptCurrInstr = BattleScript_ToxicSpikesOnTarget;
            else if (gBattlescriptCurrInstr[1] == BS_ATTACKER)
                gBattlescriptCurrInstr = BattleScript_ToxicSpikesOnAttacker;
            else
                gBattlescriptCurrInstr = BattleScript_ToxicSpikesOnFaintedBattler;
        }
    }
    ...
    else
    {
        ...
        if (hasActiveAbility(gActiveBattler, ABILITY_TRUANT))
...
static void Cmd_statusanimation(void)
{
    ...
    {
        gActiveBattler = GetBattlerForBattleScript(gBattlescriptCurrInstr[1]);
        if (!(gstatuses4[gActiveBattler] & STATUS4_SEMI_INVULNERABLE)
            && gDisableStructs[gActiveBattler].substituteHP == 0
            && !(gHitMarker & HITMARKER_NO_ANIMATIONS))
        {
...
static void Cmd_tryexplosion(void)
{
    ...
    {
        if (hasActiveAbility(gBattlerTarget, ABILITY_DAMP))
            break;
    }
...
static void Cmd_setseeded(void)
{
    if (gMoveResultFlags & MOVE_RESULT_NO_EFFECT || gstatuses4[gBattlerTarget] & STATUS4_LEECHSEED)
    {
        gMoveResultFlags |= MOVE_RESULT_MISSED;
    ...
    else
    {
        ...
        gstatuses4[gBattlerTarget] |= STATUS4_LEECHSEED;
...
bool8 UproarWakeUpCheck(u8 battlerId)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (!(gBattleMons[i].status2 & STATUS2_UPROAR) || hasActiveAbility(battlerId, ABILITY_SOUNDPROOF))
            continue;
...
static void Cmd_jumpifcantmakeasleep(void)
{
    ...
    else if (hasActiveAbility(gBattlerTarget, ABILITY_INSOMNIA)
            || hasActiveAbility(gBattlerTarget, ABILITY_VITAL_SPIRIT)
            || (hasActiveAbility(gBattlerTarget, ABILITY_LEAF_GUARD)
                    && WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_SUN)))
    {
        gLastUsedAbility = gBattleMons[gBattlerTarget].ability;
...
static u8 ChangeStatBuffs(s8 statValue, u8 statId, u8 flags, const u8 *BS_ptr)
{
    ...
    if (hasActiveAbility(gActiveBattler, ABILITY_SIMPLE))
        statValue *= 2;
    ...
    if (statValue <= -1) // Stat decrease.
    {
        ...
        else if ((hasActiveAbility(gActiveBattler, ABILITY_CLEAR_BODY)
                  || hasActiveAbility(gActiveBattler, ABILITY_WHITE_SMOKE))
                 && !certain && gCurrentMove != MOVE_CURSE)
        {
            if (flags == STAT_CHANGE_ALLOW_PTR)
        ...
        else if (hasActiveAbility(gActiveBattler, ABILITY_KEEN_EYE)
        ...
        else if (hasActiveAbility(gActiveBattler, ABILITY_HYPER_CUTTER)
...
static void Cmd_tryKO(void)
{
    ...
    {
        u16 chance;
        if (!(gstatuses4[gBattlerTarget] & STATUS4_ALWAYS_HITS))
        {
...
static void Cmd_setalwayshitflag(void)
{
    gstatuses4[gBattlerTarget] &= ~STATUS4_ALWAYS_HITS;
    gstatuses4[gBattlerTarget] |= STATUS4_ALWAYS_HITS_TURN(2);
...
static void Cmd_healpartystatus(void)
{
    ...
    if (gCurrentMove == MOVE_HEAL_BELL)
    {
        ...
        if (!hasActiveAbility(gBattlerAttacker, ABILITY_SOUNDPROOF))
        {
...
static void Cmd_trysetperishsong(void)
{
    ...
    for (i = 0; i < gBattlersCount; i++)
    {
        if (gstatuses4[i] & STATUS4_PERISH_SONG
        ...
        {
            gstatuses4[i] |= STATUS4_PERISH_SONG;
...
static void Cmd_rapidspinfree(void)
{
    ...
    else if (gstatuses4[gBattlerAttacker] & STATUS4_LEECHSEED)
    {
        gstatuses4[gBattlerAttacker] &= ~STATUS4_LEECHSEED;
        gstatuses4[gBattlerAttacker] &= ~STATUS4_LEECHSEED_BATTLER;
        BattleScriptPushCursor();
...
static void Cmd_setsemiinvulnerablebit(void)
{
    ...
    case MOVE_FLY:
    case MOVE_BOUNCE:
        gstatuses4[gBattlerAttacker] |= STATUS4_ON_AIR;
        break;
    case MOVE_DIG:
        gstatuses4[gBattlerAttacker] |= STATUS4_UNDERGROUND;
        break;
...
{
    if (gHitMarker & HITMARKER_OBEYS)
        gstatuses4[gBattlerAttacker] |= STATUS4_MINIMIZED;
...
static void Cmd_setcharge(void)
{
    gstatuses4[gBattlerAttacker] |= STATUS4_CHARGED_UP;
...
static void Cmd_tryswapitems(void)
{
    ...
    else
    {
        ...
        else if (hasActiveAbility(gBattlerTarget, ABILITY_STICKY_HOLD))
        {
            gBattlescriptCurrInstr = BattleScript_StickyHoldActivates;
            gLastUsedAbility = gBattleMons[gBattlerTarget].ability;
            RecordAbilityBattle(gBattlerTarget, gLastUsedAbility);
...
static void Cmd_trysetroots(void)
{
    if (gstatuses4[gBattlerAttacker] & STATUS4_ROOTED)
    {
        gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    }
    else
    {
        gstatuses4[gBattlerAttacker] |= STATUS4_ROOTED;
        gBattlescriptCurrInstr += 5;
    }
}
...
static void Cmd_tryGiveAquaRing(void)
{
    if (gstatuses4[gBattlerAttacker] & STATUS4_AQUA_RING)
    {
        gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    }
    else
    {
        gstatuses4[gBattlerAttacker] |= STATUS4_AQUA_RING;
        gBattlescriptCurrInstr += 5;
    }
}
...
static void Cmd_setyawn(void)
{
    if (gstatuses4[gBattlerTarget] & STATUS4_YAWN
    ...
    {
        gstatuses4[gBattlerTarget] |= STATUS4_YAWN_TURN(2);
        gBattlescriptCurrInstr += 5;
    }
}
...
static u32 getEffectiveSpeed(u8 battler)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        if ((hasActiveAbility(battler, ABILITY_SWIFT_SWIM) && gBattleWeather & B_WEATHER_RAIN)
                ...
                || (hasActiveAbility(battler, ABILITY_ICE_SKATER) && gBattleWeather & B_WEATHER_HAIL))
            speedMultiplier = 2;
        else
            speedMultiplier = 1;
    }
    ...
    else
    {
        ...
        for (battlerId = 0; battlerId < gBattlersCount; battlerId++)
        {
            if (sideAttacker != GetBattlerSide(battlerId))
            {
                ...
                {
                    gstatuses4[gBattlerAttacker] |= STATUS4_IMPRISONED_OTHERS;
                    gBattlescriptCurrInstr += 5;
                    break;
                }
            }
...
static void Cmd_trysetgrudge(void)
{
    if (gstatuses4[gBattlerAttacker] & STATUS4_GRUDGE)
    {
        gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 1);
    }
    else
...
static void Cmd_settypebasedhalvers(void)
{
    ...
    {
        if (!(gstatuses4[gBattlerAttacker] & STATUS4_MUDSPORT))
        {
            gstatuses4[gBattlerAttacker] |= STATUS4_MUDSPORT;
    ...
    {
        if (!(gstatuses4[gBattlerAttacker] & STATUS4_WATERSPORT))
        {
            gstatuses4[gBattlerAttacker] |= STATUS4_WATERSPORT;
...
static void Cmd_handleballthrow(void)
{
    ...
    else
    {
        ...
        else // mon not caught, calculate shakes
        {
            u8 shakes;

            for (shakes = 0; shakes < BALL_3_SHAKES_SUCCESS && odds > 192 / (4 - shakes); shakes++);
...
{
    if (hasActiveAbility(gBattlerTarget, ABILITY_LEAF_GUARD)
            && WEATHER_HAS_EFFECT && (gBattleWeather & B_WEATHER_SUN))
        gBattlescriptCurrInstr = T1_READ_PTR(gBattlescriptCurrInstr + 5);
    else
        gBattlescriptCurrInstr += 9;
