...
static void GiveBoxMonInitialMoveset(struct BoxPokemon *boxMon)
{
    ...
    for (i = 0; gLevelUpLearnsets[species][i].level != 0xFFFF; i++)
    {
        u16 moveLevel;
        u16 move;
...
u16 MonTryLearningNewMove(struct Pokemon *mon, bool8 firstMove)
{
    ...
    if (firstMove)
    {
        ...
        while ((gLevelUpLearnsets[species][sLearningMoveTableID].level) != (level))
        {
            ...
            if (gLevelUpLearnsets[species][sLearningMoveTableID].level == 0xFFFF)
                return MOVE_NONE;
        }
    }
...
s32 CalculateBaseDamage(struct BattlePokemon *attacker, struct BattlePokemon *defender, u32 move, u16 sideStatus, u16 powerOverride, u8 typeOverride, u8 battlerIdAtk, u8 battlerIdDef)
{
    ...
    if (WEATHER_HAS_EFFECT
            && gBattleWeather & B_WEATHER_SUN
            && hasActiveAbility2(defender, ABILITY_MAGMA_ARMOR))
        spDefense = (spDefense * 150) / 100;
    if (WEATHER_HAS_EFFECT
            && gBattleWeather & B_WEATHER_RAIN
            && hasActiveAbility2(defender, ABILITY_WATER_VEIL))
        spDefense = (spDefense * 150) / 100;
...
u8 GetMoveRelearnerMoves(struct Pokemon *mon, u16 *moves)
{
    ...
    for (i = 0; i < MAX_LEVEL_UP_MOVES; i++)
    {
        ...
        if (gLevelUpLearnsets[species][i].level == 0xFFFF)
            break;
...
u8 GetLevelUpMovesBySpecies(u16 species, u16 *moves)
{
    ...
    for (i = 0; i < MAX_LEVEL_UP_MOVES && gLevelUpLearnsets[species][i].level != 0xFFFF; i++)
         moves[numMoves++] = gLevelUpLearnsets[species][i].move;
