...
const u8 gTypeEffectiveness[336] =
{
    ...
    TYPE_POISON, TYPE_PSYCHIC, TYPE_MUL_NORMAL,
...
const u8 gCategoryNames[3][6] =
{
    [CATEGORY_PHYSICAL] = _("PHYS/"),
    [CATEGORY_SPECIAL] = _("SPEC/"),
    [CATEGORY_STATUS] = _("STAT/"),
};
...
void SwitchInClearSetData(void)
{
    ...
    if (gBattleMoves[gCurrentMove].effect != EFFECT_BATON_PASS)
    {
        ...
        for (i = 0; i < gBattlersCount; i++)
        {
            ...
            if ((gstatuses4[i] & STATUS4_ALWAYS_HITS) && gDisableStructs[i].battlerWithSureHit == gActiveBattler)
            {
                gstatuses4[i] &= ~STATUS4_ALWAYS_HITS;
    ...
    if (gBattleMoves[gCurrentMove].effect == EFFECT_BATON_PASS)
    {
        ...
        gstatuses4[gActiveBattler] &= (STATUS4_LEECHSEED_BATTLER | STATUS4_LEECHSEED | STATUS4_ALWAYS_HITS | STATUS4_PERISH_SONG | STATUS4_ROOTED | STATUS4_MUDSPORT | STATUS4_WATERSPORT | STATUS4_AQUA_RING);
        for (i = 0; i < gBattlersCount; i++)
        {
            if (GetBattlerSide(gActiveBattler) != GetBattlerSide(i)
             && (gstatuses4[i] & STATUS4_ALWAYS_HITS) != 0
             && (gDisableStructs[i].battlerWithSureHit == gActiveBattler))
            {
                gstatuses4[i] &= ~STATUS4_ALWAYS_HITS;
                gstatuses4[i] |= STATUS4_ALWAYS_HITS_TURN(2);
            }
        }
    }
    else
...
static void TryDoEventsBeforeFirstTurn(void)
{
    ...
    if (AbilityBattleEffects(ABILITYEFFECT_INTIMIDATE1, 0, 0, 0, 0) != 0)
        return;
    if (AbilityBattleEffects(ABILITYEFFECT_INTERFERENCE1, 0, 0, 0, 0) != 0)
        return;
            ...
            if (!(gBattleControllerExecFlags & ((gBitTable[gActiveBattler]) | (0xF0000000) | (gBitTable[gActiveBattler] << 4) | (gBitTable[gActiveBattler] << 8) | (gBitTable[gActiveBattler] << 0xC))))
            {
                ...
                case B_ACTION_SWITCH:
                    *(gBattleStruct->battlerPartyIndexes + gActiveBattler) = gBattlerPartyIndexes[gActiveBattler];
                    if (gBattleMons[gActiveBattler].status2 & (STATUS2_WRAPPED | STATUS2_ESCAPE_PREVENTION) || gstatuses4[gActiveBattler] & STATUS4_ROOTED)
                    {
...
u8 GetWhoStrikesFirst(u8 battler1, u8 battler2, bool8 ignoreChosenMoves)
{
    ...
    if (WEATHER_HAS_EFFECT)
    {
        if ((hasActiveAbility(battler1, ABILITY_SWIFT_SWIM) && gBattleWeather & B_WEATHER_RAIN)
            ...
            || (hasActiveAbility(battler1, ABILITY_ICE_SKATER) && gBattleWeather & B_WEATHER_HAIL))
        ...
        if ((hasActiveAbility(battler2, ABILITY_SWIFT_SWIM) && gBattleWeather & B_WEATHER_RAIN)
            ...
            || (hasActiveAbility(battler2, ABILITY_ICE_SKATER) && gBattleWeather & B_WEATHER_HAIL))
    ...
    if (getItem(battler1) == ITEM_ENIGMA_BERRY)
    {
    ...
    {
        holdEffect = ItemId_GetHoldEffect(getItem(battler1));
        holdEffectParam = ItemId_GetHoldEffectParam(getItem(battler1));
    }
    // badge boost
    if (!(gBattleTypeFlags & BATTLE_TYPE_LINK)
    ...
    if (getItem(battler2) == ITEM_ENIGMA_BERRY)
    {
    ...
    {
        holdEffect = ItemId_GetHoldEffect(getItem(battler2));
        holdEffectParam = ItemId_GetHoldEffectParam(getItem(battler2));
    }
    // badge boost
    if (!(gBattleTypeFlags & BATTLE_TYPE_LINK)
     && FlagGet(FLAG_BADGE03_GET)
    ...
    priorityBattler1 = hasActiveAbility(battler1, ABILITY_PRANKSTER) && gBattleMoves[moveBattler1].category == CATEGORY_STATUS
            ? gBattleMoves[moveBattler1].priority
            : gBattleMoves[moveBattler1].priority + 1;
    priorityBattler2 = hasActiveAbility(battler2, ABILITY_PRANKSTER) && gBattleMoves[moveBattler2].category == CATEGORY_STATUS
            ? gBattleMoves[moveBattler2].priority
            : gBattleMoves[moveBattler2].priority + 1;
...
static void HandleAction_UseMove(void)
{
          ...
          && gBattleMoves[gCurrentMove].type == TYPE_ELECTRIC)
    {
        ...
        else
        {
          ...
          && gSideTimers[side].followmeTimer == 0
          && (gBattleMoves[gCurrentMove].power != 0 || gBattleMoves[gCurrentMove].target != MOVE_TARGET_USER)
          && !hasActiveAbility(*(gBattleStruct->moveTarget + gBattlerAttacker), ABILITY_STORM_DRAIN)
          && gBattleMoves[gCurrentMove].type == TYPE_WATER)
    {
        ...
        for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++)
            if (side != GetBattlerSide(gActiveBattler)
             && *(gBattleStruct->moveTarget + gBattlerAttacker) != gActiveBattler
             && hasActiveAbility(gActiveBattler, ABILITY_STORM_DRAIN)
             && GetBattlerTurnOrderNum(gActiveBattler) < var)
                var = GetBattlerTurnOrderNum(gActiveBattler);
        if (var == 4)
        {
